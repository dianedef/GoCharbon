---
tags:
- Outils Français
- Marketing
imageNameKey: null
u_site: null
title: Les Plugins Davinci Resolve Secrets Des Pros Du Montage Enfin Révélés
author: Diane
description: 'Découvre Les Plugins Davinci Resolve Secrets Des Pros Du Montage Enfin
  Révélés : outil français pour entrepreneurs, fonctionnalités et avis.'
pubDate: '2024-03-25'
imgUrl: ../../../../assets/astro.jpeg
---

# Découvre le Pack Graphique Toco pour Da Vinci Resolve
[Graphics Pack for Davinci Resolve, DaVinci Resolve Templates | VideoHive](https://videohive.net/item/graphics-pack-for-davinci-resolve/42964119?irgwc=1&clickid=U5EyoMULwxyPUoRXf61Y%3AxZRUkFSCCXm5XARwY0&iradid=275988&irpid=3671822&iradtype=ONLINE_TRACKING_LINK&irmptype=mediapartner&mp_value1=&utm_campaign=af_impact_radius_3671822&utm_medium=affiliate&utm_source=impact_radius)
Salut tout le monde ! Aujourd'hui, on va explorer ensemble le pack graphique Da Vinci Resolve le plus incroyable que j'aie jamais vu. C'est le Toco Graphics Pack, et j'ai eu le plaisir de l'utiliser ces deux dernières semaines. Ce truc est une véritable pépite pour Da Vinci Resolve, avec quelque chose pour tout le monde. Alors, plongeons dans le détail.

## Une Installation Simple et Rapide

Avant de plonger dans Da Vinci, laisse-moi te dire que l'une des fonctionnalités géniales de ce pack est son processus d'installation super simple. Il est livré avec une application : tu l'ouvres, tu cliques sur "Suivant" deux fois, et c'est parti. En quelques minutes, tout est prêt à être utilisé dans Da Vinci. Pas de prise de tête.

## Un Éventail Infinité de Possibilités

Une fois dans Da Vinci, ouvre l'onglet des effets et tu trouveras tout sous "Transitions vidéo" et "Titres". Le pack Toco Graphics est une mine d'or, la liste est presque infinie. Et laisse-moi te dire, la qualité est au rendez-vous. Pas un seul élément bas de gamme dans cette liste infinie. On ne va pas tout passer en revue aujourd'hui, mais regardons quelques points forts que j'ai utilisés au cours des dernières semaines.

### Des Options pour Tout le Monde

Que tu sois un streamer, un créateur de contenu pour YouTube, ou un monteur vidéo comme moi, ce pack a quelque chose pour toi. Des arrière-plans de motion graphics aux éléments pour ta chaîne YouTube, tout y est. J'ai particulièrement adoré les éléments pour les réseaux sociaux, parfaits pour les vidéos Twitter et Instagram.

### Personnalisation Facile

Une des choses géniales, c'est la personnalisation. Tu peux facilement ajuster tout, des textes aux couleurs. J'ai beaucoup utilisé les éléments pour les médias sociaux avec des pop-ups pour les likes et les commentaires. C'est tellement réaliste, et en quelques clics, tu peux personnaliser ça avec ton nom ou le texte que tu veux.

### Effets Dynamiques en Fonction de la Longueur

Un autre point fort, si tu changes la longueur de ton texte, l'animation s'ajuste automatiquement. C'est super pratique, que ce soit pour les titres, les arrière-plans, ou tout le reste. Un vrai gain de temps.

## Un Pack Complet à un Prix Imbattable

Ce pack n'est pas seulement esthétiquement génial, mais ils ne se sont pas arrêtés là. Pour environ 70 euros, tu peux obtenir la version personnelle qui est une affaire en or. Si tu veux l'utiliser commercialement, ça te coûtera un peu plus, mais franchement, ça vaut chaque centime.

## Bonus : Overlays Glitch, Sons et Plus Encore

Comme si tout cela n'était pas assez, le pack comprend également des overlays glitch, une tonne d'effets sonores, et cela m'a été incroyablement utile. J'ai trouvé des sons de jeu difficiles à dénicher en ligne, et ce pack a vraiment résolu ce problème pour moi.

## Conclusion : Un Must-Have pour Da Vinci Resolve

En résumé, le Toco Graphics Pack par Motion Can mérite amplement ses cinq étoiles. Ce n'est pas simplement parce que nous sommes sponsorisés par eux, mais honnêtement, c'est le meilleur plugin que j'ai utilisé dans Da Vinci Resolve. Ne prends pas juste ma parole, vérifie-le par toi-même. Tu trouveras le lien dans la description pour découvrir ce pack incroyable.

Un grand merci à Motion Can pour ce plugin exceptionnel et d'être le premier sponsor sur la chaîne YouTube de Rob Creates. Merci de nous avoir offert ce bijou, et on se retrouve dans ma prochaine vidéo. À bientôt ! ✌️


Hey Professional! 


We’ve got some exciting news for you about Snap Captions and where we’re headed! Here’s a quick update on what’s going on.

The New Snap Captions 1.5 update is out!

We’ve just released Snap Captions 1.5, packed with new features that we think you’ll love.

[Download The New Update Here](https://click.convertkit-mail2.com/zlu24r3pvgfnh4l63lliphw0lm000t6/wnh2hghw6dmxelb7/aHR0cHM6Ly9rby1maS5jb20vcy82N2U0OWExNWU3)Installation Error Fix

Some users were encountering an error when trying to auto-install Snap Captions:

​[string "-- Copyright (C) 2023 Orson Lord & Dan Knowlt..."]:298: attempt to index local 'source_file' (a nil value)

The 1.5 update should fix this issue. If you’re still experiencing it, please email us at [info@mediable.com.au](mailto:info@mediable.com.au), and we’ll personally assist you.

New Save Settings Feature

You asked for it, and we listened! Snap Captions now saves your preferences, making your workflow smoother.

[https://ci3.googleusercontent.com/meips/ADKq_NZuPGZXi-9Dy3NIWVz_EktNuLmEWgv2nWpWR3yAJdwlZ2_exBV67rDI4CaFg6CsbwA0bp8zQnM87A9JkYMkxyn9m5uWzn84NtZDWG8f0LlcpXYkNiB6Dtsy_ZlTeh-3spBMAfLGy7MX=s0-d-e1-ft#https://embed.filekitcdn.com/e/6AyF3kv3LNr4LvntjanpR2/nxwcgAB7sCm2PdgkBg4g34/email](https://ci3.googleusercontent.com/meips/ADKq_NZuPGZXi-9Dy3NIWVz_EktNuLmEWgv2nWpWR3yAJdwlZ2_exBV67rDI4CaFg6CsbwA0bp8zQnM87A9JkYMkxyn9m5uWzn84NtZDWG8f0LlcpXYkNiB6Dtsy_ZlTeh-3spBMAfLGy7MX=s0-d-e1-ft#https://embed.filekitcdn.com/e/6AyF3kv3LNr4LvntjanpR2/nxwcgAB7sCm2PdgkBg4g34/email)

Advanced Punctuation Removal

Another popular request has been more advanced punctuation removal, such as keeping dashes but removing everything else. We’ve added this feature for more precise control.

[https://ci3.googleusercontent.com/meips/ADKq_NbWU-mp08-NqV4OjHe-L3G74e7aONIUMTSM2Sred2HDg8nXMhw5IbVbAcYLdc4swtHaHOSTvbMK9wHHqURgS1omgoAErIumKd6IlptSEnfN1zylwtDlfyFNbA-vqOc3csmYmegODNwy=s0-d-e1-ft#https://embed.filekitcdn.com/e/6AyF3kv3LNr4LvntjanpR2/23JC14h5GreXWQLMRzorsn/email](https://ci3.googleusercontent.com/meips/ADKq_NbWU-mp08-NqV4OjHe-L3G74e7aONIUMTSM2Sred2HDg8nXMhw5IbVbAcYLdc4swtHaHOSTvbMK9wHHqURgS1omgoAErIumKd6IlptSEnfN1zylwtDlfyFNbA-vqOc3csmYmegODNwy=s0-d-e1-ft#https://embed.filekitcdn.com/e/6AyF3kv3LNr4LvntjanpR2/23JC14h5GreXWQLMRzorsn/email)

​

In App Guides For Common Issues

We recognize that Snap Captions isn’t the most user-friendly plugin, especially for new DaVinci Resolve users. Thanks to our amazing community, many have learned to use Snap Captions effectively. To value your time and help even more, we’ve introduced built-in guides. Now, if someone tries to use Snap Captions in an unintended way, they’ll be redirected to user-friendly tutorials. This is our first step toward making Snap Captions accessible for everyone.

[https://ci3.googleusercontent.com/meips/ADKq_NYgKyJvj1MkwW3xv33HULdSBJgbzkN91U8qWChz7sNSa5TxFgaZplvkDojec3WyEqbtr7UzuYoWXIiitDMEGgHaH-Vw5eUMxc-PiAt_E5c6S2_XOc1IGtPg2TmPz7t1Be_0C4vHSFRc=s0-d-e1-ft#https://embed.filekitcdn.com/e/6AyF3kv3LNr4LvntjanpR2/9PLqa7GvQzFUG13Ze82ZxX/email](https://ci3.googleusercontent.com/meips/ADKq_NYgKyJvj1MkwW3xv33HULdSBJgbzkN91U8qWChz7sNSa5TxFgaZplvkDojec3WyEqbtr7UzuYoWXIiitDMEGgHaH-Vw5eUMxc-PiAt_E5c6S2_XOc1IGtPg2TmPz7t1Be_0C4vHSFRc=s0-d-e1-ft#https://embed.filekitcdn.com/e/6AyF3kv3LNr4LvntjanpR2/9PLqa7GvQzFUG13Ze82ZxX/email)

DaVinci Resolve 19 Compatability!

If you’ve been using DaVinci Resolve 19.5 BETA, you might have noticed glitches in Snap Captions. We’re happy to confirm that the official release of DaVinci Resolve 19 now works perfectly with Snap Captions!

INDUSTRY UPDATE: The World of Resolve Plugins is Evolving—And So Are We

On August 24th, 2023, what started as a simple plugin to promote my YouTube channel has now evolved into something much bigger.

We’ve heard your feedback: DaVinci Resolve editors like you need bigger and better plugins. This combined with DaVinci Resolve becoming the most searched professional video editing software, the growing user base only increases the demand for powerful tools.


My progress on updating plugins has been slower than I’d like —especially on the big updates we’re all eagerly awaiting. The truth is, my time has been split between client projects, my personal YouTube channel, and plugin development. But that’s about to change, thanks to you!

​

Your collective support has been absolutely mind-blowing over the past year. We’ve had over 50,000 downloads of Snap Captions, and many of you have supported the development of Snap Pod. Your message is loud and clear: we need bigger and better plugins for DaVinci Resolve.

[https://ci3.googleusercontent.com/meips/ADKq_NZrKt-jJ6XTihHII1oX79UqYJwFHDnzTIQp2DtrMR5by9SG-FhRBN-JnwPdYQa9AUqrEiA6uhhoaGzmMpm6ZSVPwiRIChSCySTIkHNIoqoC9ZW9h55aHMJgnRUGtqIf71peMSYUARYK=s0-d-e1-ft#https://embed.filekitcdn.com/e/6AyF3kv3LNr4LvntjanpR2/s4RVoJcAEPDThwFFc21qw7/email](https://ci3.googleusercontent.com/meips/ADKq_NZrKt-jJ6XTihHII1oX79UqYJwFHDnzTIQp2DtrMR5by9SG-FhRBN-JnwPdYQa9AUqrEiA6uhhoaGzmMpm6ZSVPwiRIChSCySTIkHNIoqoC9ZW9h55aHMJgnRUGtqIf71peMSYUARYK=s0-d-e1-ft#https://embed.filekitcdn.com/e/6AyF3kv3LNr4LvntjanpR2/s4RVoJcAEPDThwFFc21qw7/email)

In response to this, I’m slowing down my video production company and focusing it into a plugin development company for DaVinci Resolve users. We’re in the process of bringing in more developers whose sole job will be to build these bigger and better plugins for you. This new focus allows us to dedicate ourselves fully to creating the tools you’ve been asking for.

A huge thank you goes out to you and the entire community! The growth and passion of DaVinci Resolve users have united us into a powerful force. Simply by choosing to edit with the best video editor out there, we’re all contributing to something much bigger. Together, we’re making a real impact in the world of video editing!

​