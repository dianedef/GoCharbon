...
- WriterZen - Qu'est-ce que la difficulté des mots clés? Comment est-il marqué?
    
    Qu'est-ce que la difficulté du mot-clé? Comment est-il marqué? Ha Tuan Khang | 10 minutes de lecture
    
    Le défi Lors de la mise en œuvre d'un projet SEO, nous devons toujours effectuer des recherches par mots clés et sélectionner les mots clés appropriés en fonction des capacités, des ressources et de la stratégie SEO que nous choisissons. Et il est très important de déterminer la difficulté d'un mot-clé (difficulté des mots-clés). Nous avons recherché et analysé cette métrique fournie par de nombreux autres partis tels que Ahrefs et Semrush. Mais il y a un inconvénient important à cet égard - ils ne déterminent la difficulté d'un mot clé que par l'impact du nombre de domaines pointant vers le domaine qui se classe pour un mot-clé particulier. Pour certains évaluateurs, plus il y a de domaines référents sur les pages de premier rang, plus la difficulté du mot clé est grande.
    
    L'équipe de recherche de WriterZen ne le pense pas. La difficulté d'un mot clé est non seulement affectée par chaque effet de lien (plus il y a de domaines référents, meilleure est la qualité), mais elle dépend également de nombreux autres facteurs.
    
    # **Algorithme de difficulté de mot-clé dans WriterZen**
    
    
    
    Après avoir analysé, mesuré et expérimenté de nombreux projets, nous avons constaté que la difficulté d'un mot-clé est également affectée par d'autres facteurs tels que:
    
    - La quantité de contenu écrit sur ce mot-clé.Le niveau de concurrence lors de l'exécution de Google Ads.Le nombre de mentions sur la [marque.La](http://marque.La) quantité de trafic sur le marché affecte directement la concurrence, c'est-à-dire la difficulté du mot-clé.
    
    > KD moyenne = KD par backlink * 25% + KD par contenu * 20% + KD par trafic * 25% + KD par signaux * 20% + KD par CPC * 10% / 5 * 100%
    
    Il existe de nombreux exemples pour soutenir nos recherches. Certains mots clés ont une très faible difficulté de liaison, selon une analyse d'Ahrefs ou Semrush. Mais en réalité, lors de la mise en œuvre de ces mots clés, nous voyons que ce n'est pas aussi facile que nous le pensons sur la base du KD qu'Ahrefs donne. Ce sont des mots clés difficiles à concurrencer car il y a trop de parties qui écrivent des articles à ce sujet (KD sur le contenu est très élevé), beaucoup de gens s'y intéressent, ce qui conduit de nombreuses parties à investir dans ce rang de mots clés, qui a un trafic très élevé .
    
    Il y a une chose que nous, des débutants aux experts en référencement, comprenons tous: la mise en œuvre d'un projet de référencement avec chaque personne, chaque équipe et chaque entreprise a des stratégies différentes. Parce que chacun de nous a divers facteurs humains, des ressources supplémentaires et des forces. Il y a des partis extrêmes dans le contenu, il y a des partis très forts avec de nombreuses ressources de liaison, et il y a des parties avec des ressources de trafic importantes. Nous ne pouvons pas construire le bon plan de référencement si nous sommes forts en contenu mais utilisons KD sur les liens pour évaluer la difficulté du projet.
    
    Pour être le bon stratège, nous devons analyser tous les facteurs. Nous devons ensuite utiliser nos ressources de la manière la plus raisonnable. Et croyez-nous, nous pouvons parfois être au sommet même si nous ne sommes pas forts dans les liens, mais forts dans des facteurs opposés. Mais pour mettre en œuvre des stratégies pour y parvenir, nous avons certainement besoin d'une métrique KD qui reflète mieux la difficulté du mot-clé à tous égards. C'est pourquoi WriterZen a pris le risque d'analyser et de trouver une nouvelle façon de déterminer KD, qui est une fusion d'évaluations basées sur le contenu KD, mentionner KD, Google Ads KD, Lien KD, et trafic KD. Avec cette évaluation plus approfondie, nous vous donnerons les mesures les plus précises sur la concurrence d'un mot-clé.
    
    Rappel: La difficulté des mots clés ne mesure que vos chances d'entrer dans le top 10 du SERP (pas dans le top 3 ou le top 1).
    
    Échelle de difficulté de mot-clé WriterZen propose une échelle de difficulté de mot clé de 0 à 100 Plus l'indice KD est élevé, plus la difficulté est élevée. Et la difficulté des mots clés évalue différents facteurs tels que le contenu, la mention, le lien, Google Ads CPC ou le trafic qui ne sont pas linéaires les uns avec les autres.
    
    1. Difficulté par mot-clé par lien Voici la relation entre Difficulté par mot-clé par lien et le nombre correspondant de domaines de référence nécessaires :
        
        
        
    
    L'algorithme pour déterminer la difficulté du mot clé par contenu est basé sur le nombre total d'articles sur tous les sites Web du marché d'un pays particulier sur un mot-clé. De plus, il est basé sur l'analyse de 100 concurrents sur le SERP en termes de quantité et de qualité de contenu dont disposent ces concurrents. À partir de là, nous pouvons obtenir Keyword Difficulty by Content, sur la base duquel nous avons une base pour imaginer le niveau d'investissement dans le contenu.
    
    1. Difficulté par mot-clé par mention (ou signal) Vous trouverez ci-dessous la relation entre Difficulté par mot-clé par mention (ou signal) et le montant correspondant de la mention nécessaire :
    
    
    
    L'algorithme Difficulté par mention (reflétant la popularité de la marque associée au site Web) est déterminé en fonction de la mesure du nombre de fois où les utilisateurs mentionnent les marques des concurrents apparaissant sur le SERP. Plus le nombre de mentions est élevé, plus le mot clé est compétitif. Notre analyse est basée sur la mesure de 300 millions de sites Web. Il fait une recommandation sur le nombre correspondant de mentions nécessaires à construire pour placer le mot clé dans le top 10 sous l'influence du facteur mentionné sur votre marque.
    
    1. Mot clé Difficulté par le trafic Vous trouverez ci-dessous la relation entre Mot clé Difficulté par le trafic et la quantité correspondante de trafic organique nécessaire :
    
    L'algorithme pour déterminer la KD par le trafic est basé sur la détermination du niveau de concurrence et de la quantité de trafic de recherche organique qui doit être construit pour se classer dans le top 10 avec n'importe quel mot-clé.
    
    Conclusion
    
    Analyser et choisir les bons mots clés pour la bonne compétition est un travail qui n'a jamais été simple. WriterZen, avec un mécanisme de détermination des difficultés des mots clés basé sur de nombreux facteurs, vous aidera à avoir la bonne perspective et vous aidera à augmenter la probabilité de choisir des mots clés qui vous apportent un bon trafic sans avoir besoin de trop d'efforts pour classer. Il vous aidera également à tirer le meilleur parti des ressources dont vous disposez, soit en termes de contenu, de liens, de mentions ou de trafic.
...
- How to SEO Keyword Research - Beginner's Guide to SEO [2022]
    ...
    Search engines such as [Google](https://www.google.com/), [Bing](https://www.bing.com/), and [Yandex](https://yandex.com/) use keywords to rank the search engine results.
    ...
    _For example, if I write a blog post on “[Best Practices in SEO 2021: Evergreen Search Engine Optimization Principles & Tips](https://digitalmarketingmind.com/best-practices-in-seo/)“, then “SEO” is my seed keyword._
    
    The primary keyword in SEO is your target or popular phrase or a word that you want to rank high for in search engines.
    
    _For example, if I am writing a blog post on “Best Practices in SEO 2021: Evergreen Search Engine Optimization Principles & Tips”, then “Best Practices in SEO” is my primary keyword._
    ...
    _For example, if I am writing a blog post on “Best Practices in SEO 2021: Evergreen Search Engine Optimization Principles & Tips”, then one of my secondary keywords is “best places to buy the domain name”._
    
    The first important thing is that there is no such thing as “LSI keywords”; it’s an improper term we use.
    
    The acronym “LSI” stands for [Latent Semantic Indexing](https://nlp.stanford.edu/IR-book/html/htmledition/latent-semantic-indexing-1.html).
    ...
    _For example, words such as “phone” and “apple” might have two different meaning, but when we put together, then it points to “Apple brand phone.”_
    ...
    _For example, suppose I am writing a blog post on “Best Practices in SEO 2021: Evergreen Search Engine Optimization Principles & Tips”. In that case, one of my LSI keywords is “Namecheap,” which is one of the domain registrars I recommended in that article._
    ...
    _For example, This includes movie name or incident._
    ...
    _For example, ‘Learn SEO for free’ is a long-term evergreen keyword because it has been searched for decades and will be in the future too._
    
    Seasonal keywords are those that have a certain time frame in which they are relevant.
    
    _For example, ‘Christmas gifts’ is a seasonal keyword as it would be relevant for the time period from November to December._
    ...
    _For example, a product from a company that closed down recently or people is moving away from an industry like “Film-roll camera to digital.”_
    ...
    _For example, the keyword “SEO” is a market segment keyword because it is associated with the Search Engine Optimization industry._
    ...
    _For example, a customer-oriented keyword might be “best web hosting for small business owners” This keyword is derived from research on niche and target audience._
    
    ### Product defining keywords
    
    Product and service keywords are words or phrases that describe a product or service.
    
    _For example, a product or service keyword might be “WordPress hosting.”_
    
    Brand keywords are words that are associated with a particular brand.
    
    _For example, Google is known for its search engine, while Apple is known for its products._
    ...
    _For example, for Bing, a competitor keyword might be “Google search engine” or “Best search engine” if Google is targeting that keyword._
    ...
    _For example, someone might type “drink” as “drik,” and search engine optimization is known as “SEO.”_
    ...
    _For example, creating a blog post about “SEO guide,” will be an informational keyword because it can help people learn about SEO._
    ...
    _For example, if you create a page about “Semrush keyword tool,” this will be a navigational keyword because searcher know what they want to know more._
    ...
    _For example, creating a blog post about “Semrush review” or “Semrush discount” then will be a transactional keyword because they understand the problem, know the solution, and pick a specific product for them._
    ...
    - [Google Panda](https://en.wikipedia.org/wiki/Google_Panda): Keyword stuffing, which is just adding all keywords in the text without any context or logic.
    - [Google Hummingbird](https://en.wikipedia.org/wiki/Google_Hummingbird): Understanding the meaning and user intent behind search queries
    - [RankBrain](https://en.wikipedia.org/wiki/RankBrain): Better search intent understanding and machine learning
    - [Google Possum](https://moz.com/learn/seo/google-possum): Better results based upon the searcher’s location and address of the business
    ...
    **Download Template**
    ...
    Hey, I'm Alston, one of the guys behind Digital Marketing Mind. I run a digital agency and share my 10+ years of knowledge on blogging, SEO, and digital marketing. You can read [about me here](https://digitalmarketingmind.com/about/).
    
    Join our amazing community of more than 5400+ digital entrepreneurs to help each other on digital marketing and SaaS deals.
    
    [Join our FB Community ➜](https://www.facebook.com/groups/alstonantony)
...
- Qui a la plus "growth" pour choisir les mots-clés ? (1ère partie) - Réacteur
    ...
    ## La recherche de mots clés, dans le cadre d'une stratégie SEO, nécessite bien sûr d'avoir à sa disposition les outils adéquats pour mener à bien ses investigations. Mais il s'agit également, qui plus est dans une démarche plus spécifiquement B2B, d'appréhender finement les métiers traités et les contextes abordés. Car sans cela, on peut vite passer à côté du trafic escompté...
    
    
    
    Rassurez-vous : il ne s’agit pas d’un concours trivial ni d’un propos nourrissant une forme de clivage entre métiers. L’objet de cet article est simplement de pousser les parties prenantes d’un projet de site web à l’introspection. Objectif : mieux travailler ensemble dans un intérêt de croissance de l’entreprise et dans un esprit collaboratif. Voyons pourquoi et dans quel contexte la recherche et la recommandation de mots-clés ne sont pas forcément un jeu d’enfant ou le pré carré de certains spécialistes.
    
    ### Comprendre l’activité pour de vrai
    
    Si en B2C, l’activité de vente en ligne de chaussettes reste assez simple à cerner, la compréhension se corse radicalement lorsqu’il s’agit d’un éditeur de logiciel B2B dont les tenants et aboutissants prennent un certain temps à intégrer.
    
    Prenons par exemple le marché des solutions SaaS PIM (_Product Information Management_ ou gestion de l’information produit en français). La diversité des plateformes proposées est tellement disparate qu’elle oblige à choisir et/ou à écarter certains mots-clés, selon les possibilités ou impossibilités fonctionnelles du logiciel concerné.
    
    Nous pouvons ainsi nous interroger légitimement sur le degré de connaissance des responsables SEO des sites s’autoproclamant "producteurs de leads" concernant ce type d’univers spécifique, donc de leurs capacités à cibler les meilleurs mots-clés dans un contexte qui a pour vocation à générer des prospects de qualité.
    
    Et ce d’autant plus lorsque les sujets complexes à traiter sont légion… et dans des univers métiers totalement différents !
    
    Sans cette compréhension incontournable, la recommandation des mots-clés n’est pas optimale :
    
    - Elle peut comporter des erreurs et proposer des expressions "inutiles", car le logiciel n’offre pas certaines fonctionnalités qui correspondent à ces mots-clés ;
    - La recommandation peut passer à côté d’une multitude d’opportunités, par pure ignorance ;
    - Elle peut être induite (et induire) en erreur en comportant des termes "faux amis", ou encore "erronés" du fait d'une nouvelle réglementation en vigueur, du changement des usages et même du contexte économique.
    
    Le contexte Covid a notamment engendré une augmentation des ventes en ligne de + 45,5 % dans le domaine du e-commerce (source : [Republik Retail](https://www.republik-retail.fr/strategie-retail/enseignes/quelles-sont-les-nouvelles-habitudes-d-achat-alimentaire-des-consommateurs-etude.html)). Pour qui connaît l’utilité fonctionnelle du PIM côté e-commerce, on saura quoi faire de cette information…
    
    La veille sur le sujet apparaît donc également importante pour saisir ou "appuyer" certaines opportunités de mots-clés.
...
- Qui a la plus "growth" pour choisir les mots-clés ? (2ème partie) - Réacteur
    ...
    [Notre premier article sur le sujet](https://www.reacteur.com/2021/07/qui-a-la-plus-growth-pour-choisir-les-mots-cles-1ere-partie.html) avançait la conclusion suivante : seul, on va plus vite, ensemble, on va plus loin ! Sortez les mouchoirs tellement c’est beau ! Séquence émotion motivante à partager en petit groupe intime … et bosseur ! OK, mais concrètement ? Retroussons-nous les manches : [ce billet](https://fr.semrush.com/blog/content-marketing-b2b/) nous détaille les 20 atouts d’une équipe éditoriale interne pour une entreprise B2B. Certains arguments constituent une réelle source d’inspiration pour se poser les bonnes questions : identifier les bons profils et moyens permettant d’acquérir de la connaissance à tous les niveaux pertinents. Objectifs : enrichir sa recherche de mots-clés pour aboutir à une "growth" recommandation !
    
    ### Qui sait précisément ce que fait l’entreprise ? Quelles questions poser ?
    
    Lorsque ce que vend une entreprise B2B parait _a priori_ complexe, le premier pilier de la connaissance consiste à comprendre son activité dans les moindres détails. Pour cela, rien de mieux que de questionner et d’échanger avec les experts métiers en interne tels que :
...
[Understand The Supply & Demand Principle In Google's Economy | Your Role As Content Creator in 2022 - YouTube](https://www.youtube.com/watch?v=5ArcTl5BOwY)

LSI Graph, Ultimate Keyword Hunter et Answer The Public. Sinon, vous pouvez tout simplement utiliser [Google Suggest](https://audreytips.com/glossaire-web/google-suggest/).

- BrightonSEO July2021 Psychology in Search (1)
    ...
    - Ressources
    - Todo's 1.
- WriterZen - ESKP - An important metric you cannot miss when doing keyword research (1)
