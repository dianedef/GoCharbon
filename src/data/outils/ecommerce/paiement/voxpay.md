...
**Liens de paiement omnicanaux** — Voxpay Link te permet de générer des liens de paiement et de les envoyer par SMS, email, WhatsApp, chat ou QR code. Le client clique, paye en 3 secondes, et tu reçois une confirmation instantanée. Idéal pour les devis, les acomptes, les factures à distance.

**Recouvrement automatisé** — Voxpay Collect automatise l'envoi de relances de paiement (liens, SMS, emails) pour récupérer les impayés. Tu configures des scénarios (relance J+7, J+15, J+30) et Voxpay envoie automatiquement les liens de paiement. Résultat : meilleur taux de recouvrement sans effort manuel.

**PCI-DSS 4.0.1 — le plus haut standard** — Voxpay est certifié PCI-DSS 4.0.1 Niveau 1, la version la plus récente et la plus exigeante du standard de sécurité des paiements. Les données sont chiffrées, hébergées en France, et conformes au RGPD.
...
- Services de recouvrement et de relance d'impayés
- Agences immobilières (arrhes, dépôts de garantie à distance)
- Hôtels et hébergements (pré-autorisation, paiement à distance)
...
3. **Intégration** — Connecte Voxpay à ton PSP ou contrat VAD existant (offre Business), ou utilise directement le PSP intégré (offre Essentiel)
...
- **QR code** — Génération de QR codes de paiement
...
|-------|-----------------|--------|
| **Essentiel** | 1% + 0,20€ HT | Premiers paiements en ligne en 1 minute, services Voxpay Digital inclus |
| **Business** | Sur devis | API, paiement vocal, liens personnalisés, campagnes de recouvrement, intégration PSP/VAD |

- **Pas de frais d'installation** sur l'offre Essentiel
- **Essai gratuit** disponible
- **Offre Business** — Personnalisée selon le volume et les canaux utilisés
...
- **API** : API pour les intégrations personnalisées (offre Business)
- **CRM** : Connexion avec les outils de gestion client
- **SMS/Email** : Envoi de liens de paiement automatisé
...
- **2020-2022** — Développement des produits : Voxpay Assist (paiement vocal), Voxpay Link (liens de paiement), Voxpay Selfcare (SVI), Voxpay Collect (recouvrement). L'offre s'élargit du paiement vocal vers une plateforme omnicanale
- **2023** — Agrément **ACPR** comme Prestataire de Services de Paiement (PSP). Voxpay peut désormais traiter des paiements directement, sans dépendre d'un PSP tiers
- **2024-2026** — Certification **PCI-DSS 4.0.1** (la version la plus récente). Lancement du **SoftPOS** (TPE virtuel sur smartphone). Lancement de l'offre **Essentiel** (1% + 0,20€, démarrage en 1 minute) pour démocratiser l'accès aux petites entreprises. Voxpay continue de recruter et se positionne comme le leader français du paiement à distance sécurisé
...
**L'artisan qui encaisse ses devis à distance** — Tu es plombier. Tu envoies un devis de 450€ par email. Au lieu d'attendre un virement (5 jours) ou un chèque (10 jours), tu joins un lien Voxpay dans l'email. Le client paye en 3 clics par carte bancaire. L'argent est sur ton compte sous 48h.
...
- Agréé ACPR comme PSP
- Données hébergées en France (RGPD)
- Campagnes de recouvrement automatisées (Voxpay Collect)
- Paiement vocal sécurisé — coordonnées bancaires masquées
- Pricing transparent : 1% + 0,20€ (offre Essentiel)
...
- **Pas de TPE physique** — Voxpay est une solution de paiement à distance. Si tu as besoin d'un terminal de paiement physique en magasin, il faut un autre fournisseur (SumUp, Yavin)
- **Offre Essentiel limitée** — L'offre à 1% + 0,20€ couvre les liens de paiement mais pas le paiement vocal ni le recouvrement automatisé. Pour les fonctionnalités avancées, il faut passer à l'offre Business (sur devis)
- **Peu d'informations publiques** — Pas de chiffres sur le nombre de clients, le volume de transactions ou le CA. Difficile d'évaluer la traction
- **Pas de reviews publiques** — Aucun avis significatif sur Trustpilot, G2 ou Capterra
